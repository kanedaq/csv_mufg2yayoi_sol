// 三菱東京ＵＦＪ銀行の取引csvを、やよいの青色申告に取り込めるcsvに変換
...
int main(int argc, char* argv[])
{
	try {
		...
		for (const auto& record : records)
		{
			bool shiharai = true;
			std::string kingaku;
			if (2 < record[3].size() && 2 == record[4].size()) {
				kingaku = record[3];
			}
			else if (2 == record[3].size() && 2 < record[4].size()) {
				shiharai = false;
				kingaku = record[4];
			}
			else {
				throw MyException("支払い金額or預かり金額の長さがおかしい");
			}
			...
			if (record[2].size() <= 2) {
				...
				fo << record[1].substr(0, record[1].size() - 1) << "：" << record[2].substr(1) << ',';
			}
		...
		std::cout << "csv_mufg2yayoi Version 1.0\n";
		std::cout << "Usage: csv_mufg2yayoi in_csv out_csv" << std::endl;
		return 1;
	}
